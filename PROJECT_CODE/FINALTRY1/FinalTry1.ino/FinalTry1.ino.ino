...
float distance = 50;                     //Distance between two seeds = 20cm
float columns = 200;                          //Length Of column = 200cm
int col = 4;                      //Number of Columns = 4
float distBet = 20;                   //Distance between two columns = 50cm
...
float turn = 1.5; //(double to float)            //Time taken by bot to take complete 90degree turn(Assuming that right turn time = left turn time)
...
void loop() {
  ...
  if (p == 0) {
    ...
    if ( (key == '=') && (i == 4) && (d == 1) ) {
      ...
      lcd.setCursor(3, 1);
      lcd.print("Thank You!");
      delay(1000);                  //Delay after taking inputs from user
      lcd.clear();
      lcd.print("Distance :");
      lcd.print(distance);
      lcd.setCursor(0,1);
      lcd.print("Length of Col :");
      lcd.print(columns);
      delay(2000);
      lcd.clear();
      lcd.print("No. of col : ");
    ...
    if (key != '*' && key != 'A' && key != 'B' && a == 1) {
      ...
      if (key == 'C' && b==0) {             //If loop to erase everything
        distance = 0;
        m = 0;
        decDist = 0;
        mult = 10;
    ...
    if (b == 1 && key != 'A' && key != 'B' && key != '=') {
      ...
      if (key == 'C' && c==0) {             //If loop to erase everything
        columns = 0;
        n = 0;
        mult = 10;
        decColumns = 0;
    ...
    if (c == 1 && key != 'B' && key != '=' && key != '.') {
      ...
      if (key == 'C' && d==0) {             //If loop to erase everything
        col = 0;
        print("No. of columns");
      }
    }
...
if((p == 2)){
    ...
    if (temp < r) {
      ...
      //first half cycle
      if ((temp == s && Nmillis >=  x + (2 * turn) + u ) || r == 0){
          off();
        }
